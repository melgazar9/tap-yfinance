...
class TapYFinance(Tap):
    ...
    config_jsonschema = th.PropertiesList(
        th.Property(
            "financial_category",
            th.ObjectType(additional_properties=th.ObjectType()),
            required=True,
        ),
    ).to_dict()
    ...
    def discover_catalog_entry(
        financial_category: str, table_name: str, schema: dict
    ) -> CatalogEntry:
    ...
    def catalog_dict(self) -> dict:
        ...
        if (
            hasattr(self, "_catalog_dict") and self._catalog_dict
        ...
        financial_categories: dict[str, dict[str, dict]] = self.config.get(
            "financial_category", {}
        )
        ...
        for fc in financial_categories.keys():
            for table_name in financial_categories[fc].keys():
                schema = get_schema(
                    financial_categories[fc][table_name]["schema_category"]
                )
                catalog_entry: CatalogEntry = self.discover_catalog_entry(
                    financial_category=fc, table_name=table_name, schema=schema
                )
    ...
    def discover_streams(self) -> list:
        ...
        for catalog_entry in self.catalog_dict["streams"]:
            financial_category = catalog_entry["metadata"][-1]["metadata"][
                "schema-name"
            ]
            if financial_category == "prices" and not catalog_entry[
                "tap_stream_id"
            ].endswith("wide"):
                stream = PriceStream(self, catalog_entry=catalog_entry)
            elif financial_category == "prices" and catalog_entry[
                "tap_stream_id"
            ].endswith("wide"):
